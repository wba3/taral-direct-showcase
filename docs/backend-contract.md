...
`src/lib/adapters/types.ts` sketches the read and validation boundary. Operational
workflows currently call `src/lib/demo-store.tsx` directly. Production integration
requires implementing server adapters and wiring those store actions to authenticated
requests, including loading, retry and failure states; replacing the mock module alone
is insufficient.
...
| `sample_requests`        | `reference`, `account_id` (nullable for guests), `idempotency_key` unique, `status`, `contact`, `items`                                     | server-controlled guest intake (rate limited + abuse checks); read by verified owner or staff       |
